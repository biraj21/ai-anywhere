...
async function init() {
	if (!chrome.runtime?.id) {
		logger.error(
			"Extension context invalidated at init. Skipping initialization.",
		);
		return;
	}
	...
	try {
		const { floatingSelectionButtonEnabled } = await chrome.storage.local.get(
			"floatingSelectionButtonEnabled",
		);
		if (floatingSelectionButtonEnabled !== true) {
			logger.debug(
				"Selection button not enabled in settings. Skipping initialization.",
			);
			return;
		}
	} catch (error) {
	...
	const menuHasDraft = () => {
		...
		const customInput = selectionMenu.shadowRoot.querySelector(
			".custom-input",
		) as HTMLInputElement | null;
		const speechTextarea = selectionMenu.shadowRoot.querySelector(
			".speech-textarea",
		) as HTMLTextAreaElement | null;

		return Boolean(
			customInput?.value.trim().length || speechTextarea?.value.trim().length,
		);
	};
	...
	const isClickOnButtonOrMenu = (target: EventTarget | null) => {
		...
		return Boolean(
			selectionMenu &&
				(target === selectionMenu || selectionMenu.contains(target as Node)),
		);
	};

	const submitSelection = async (
		text: string,
		formatType: ContextMenuValue = ContextMenu.AiAnywhere,
		customPrompt?: string,
	) => {
		isMenuOpen = false;
	...
	const setupMenuEventListeners = (menu: HTMLDivElement) => {
		...
		const menuContainer = menuShadow.querySelector(
			".menu-container",
		) as HTMLElement;
		const customInput = menuShadow.querySelector(
			".custom-input",
		) as HTMLInputElement;
		const templateList = menuShadow.querySelector(
			".template-list",
		) as HTMLDivElement;
		...
		const loadTemplates = async () => {
			try {
				const { promptTemplates } =
					await chrome.storage.local.get("promptTemplates");
				...
				templateList.style.display = speechPrompt.isVoiceMode()
					? "none"
					: "flex";
	...
	document.addEventListener("keyup", (event) => {
		if (
			...
			event.altKey ||
			event.metaKey ||
			!textSelectionKeys.includes(event.key)
		) {
			return;
		}
